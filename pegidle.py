...
SETTINGS_FILE = "peggle_settings.json"
...
def create_bite_sound(filename="bite.wav"):
    if not os.path.exists(filename):
        sample_rate, duration = 44100, 0.15
        wavef = wave.open(filename, 'w')
        wavef.setnchannels(1)
        wavef.setsampwidth(2)
        wavef.setframerate(sample_rate)
        for i in range(int(sample_rate * duration)):
            value = int(32767.0 * (random.random() * 2.0 - 1.0) * 0.8) # Crunchy noise
            envelope = (1.0 - (i / (sample_rate * duration)))**4 # Very sharp decay
            data = struct.pack('<h', int(value * envelope))
            wavef.writeframesraw(data)
        wavef.close()
...
# --- LOCAL SETTINGS (AUTO LOGIN) ---
def save_local_settings(user, pw):
    try:
...
def send_to_gas(payload):
    try:
        ...
        req = urllib.request.Request(GAS_WEB_APP_URL, data=data, headers={'Content-Type': 'application/json'})
...
def get_save_data_dict():
    return {
        ...
        'ability_inventory': ability_inventory, 'boards_cleared': boards_cleared,
...
def save_game(sync=False):
    ...
    if current_user and current_pass:
        ...
        if sync:
            print("Force saving to cloud before exit...")
            send_to_gas(payload)
        else:
            threading.Thread(target=send_to_gas, args=(payload,), daemon=True).start()
...
def auth_thread(action, username, password):
    ...
    payload = {"action": action, "username": username, "password": password, "saveData": saveDataStr}
    res = send_to_gas(payload)
    
    if res.get("success"):
        current_user = username
        current_pass = password
        save_local_settings(username, password)
        
        if action == "login":
            cloud_save_str = res.get("saveData", "{}")
            if cloud_save_str:
                try: apply_save_data(json.loads(cloud_save_str))
                except: pass
        
        check_offline_progress()
        if state in ["LOGIN", "AUTO_LOGIN"]: 
            state = "PLAY"
    else:
...
# --- INITIATE AUTO LOGIN ---
saved_creds = load_local_settings()
if saved_creds and saved_creds.get("username") and saved_creds.get("password"):
    state = "AUTO_LOGIN"
    is_authenticating = True
    input_username = saved_creds["username"]
    input_password = saved_creds["password"]
                ...
                for _ in range(total_drops):
                    ...
                    for peg in hit_pegs:
                        ...
                        if peg['type'] == 'prestige': sim_pp += 3
                            ...
                            if base_val > 0: sim_cash += int(base_val * b_stat['gold_mult'])
...
def generate_tree_nodes():
    ...
    branches = [
        ('starter_cash', angles[0], GREEN), ('extra_rainbow', angles[1], CYAN), ('bomb_chance', angles[2], ORANGE),         
        ('extra_prestige', angles[3], BROWN), ('gold_chance', angles[4], GOLD), ('multishot', angles[5], PURPLE), ('stat_pegs', angles[6], WHITE) 
    ]
...
def spawn_particles(x, y, color, count=15, speed=4.0):
    for _ in range(count):
        particles.append({
            'x': x, 'y': y, 'vx': random.uniform(-speed, speed), 'vy': random.uniform(-speed, speed),
...
def spawn_ball(ball_type, x, y, vx, vy, is_manual=False, inherited_mult=None):
    ...
    b_dict = {
        'type': ball_type, 'is_manual': is_manual, 'x': x, 'y': y, 'vx': vx, 'vy': vy,
        'color': base['color'], 'grav': base['base_grav'], 'bounce': final_bounce, 'radius': radius,
        'gold_mult': inherited_mult if inherited_mult else stats['gold_mult'], 'revive_stack': revive_stack 
    }
    
    if ball_type == 'Magic':
        b_dict['lightning_strikes'] = 2; b_dict['lightning_timer'] = FPS * 1.5; b_dict['vy'] = 0.0 
    elif ball_type == 'Taco':
        b_dict['max_hits'] = random.randint(2, 4)
        b_dict['hits_taken'] = 0
        b_dict['radius'] = 14 # Tacos are a bit chunky
...
def draw_main_menu():
    ...
    btn_logout = pygame.Rect(m_x + 20, m_y + 20, 100, 40)
    pygame.draw.rect(screen, RED, btn_logout, border_radius=5)
    screen.blit(font_med.render("Logout", True, WHITE), (btn_logout.x + 10, btn_logout.y + 10))

    stats = ball_stats[equipped_ball]
    pygame.draw.rect(screen, TAN, (m_x + 30, m_y + 80, menu_w - 60, 110), border_radius=5)
    screen.blit(font_large.render(f"[{equipped_ball}] Lvl {stats['level']}", True, BLACK), (m_x + 50, m_y + 90))
    screen.blit(font_med.render(f"GOLD: x{stats['gold_mult']:.1f}", True, BLACK), (m_x + 50, m_y + 130))
    screen.blit(font_med.render(f"MANUAL AMMO: {stats['max_balls']}", True, BLACK), (m_x + 50, m_y + 160))
    
    btn_power = pygame.Rect(m_x + 400, m_y + 90, 250, 40)
    ...
    btn_cap = pygame.Rect(m_x + 400, m_y + 140, 250, 40)
    ...
    btn_special = None; btn_top_rev = None; btn_bot_rev = None
    top_lv = stats.get('base_top_revives', 0); bot_lv = stats.get('base_bounce_revives', 0)
    top_cost = top_lv + 1; bot_cost = bot_lv + 1
    
    if not stats.get('special_unlocked'):
        btn_special = pygame.Rect(m_x + 680, m_y + 90, 230, 30)
        ...
        screen.blit(font_small.render(f"Top Revives: {top_lv}/5", True, DARK_BLUE), (m_x + 680, m_y + 95))
        btn_top_rev = pygame.Rect(m_x + 820, m_y + 90, 60, 25)
        if top_lv >= 5:
        ...
        screen.blit(font_small.render(f"Bounce Rev: {bot_lv}/5", True, DARK_BLUE), (m_x + 680, m_y + 120))
        btn_bot_rev = pygame.Rect(m_x + 820, m_y + 115, 60, 25)
        if bot_lv >= 5:
    ...
    for i, (b_name, b_data) in enumerate(visible_balls.items()):
        ...
        if b_name == "Taco":
            tx, ty = card.x + 67, card.y + 25
            pygame.draw.circle(screen, GOLD, (tx, ty), 15)
            pygame.draw.rect(screen, BROWN, (tx - 13, ty - 2, 26, 6))
            pygame.draw.rect(screen, GREEN, (tx - 11, ty - 6, 22, 4))
        else:
        ...
        if len(desc_words) > 3:
            screen.blit(font_tiny.render(" ".join(desc_words[:3]), True, DARK_GRAY), (card.x + 5, card.y + 70))
            screen.blit(font_tiny.render(" ".join(desc_words[3:]), True, DARK_GRAY), (card.x + 5, card.y + 85))
        else:
        ...
        if ball_stats[b_name]['unlocked']: screen.blit(font_small.render("EQUIPPED" if equipped_ball == b_name else "EQUIP", True, GREEN), (card.x + 15, card.y + 100))
            ...
            cost_str = f"${b_data['cost']//1000000}M" if b_data['cost'] >= 1000000 else f"${b_data['cost']}"
            screen.blit(font_small.render(cost_str, True, RED), (card.x + 15, card.y + 100))
            
        # Draw Auto Upgrade Button in Menu (if unlocked)
        if ball_stats[b_name]['unlocked']:
            btn_auto = pygame.Rect(card.x + 5, card.y + 10, 30, 20)
            c_stat = ball_stats[b_name]
            if c_stat['auto_drop_lvl'] >= 4:
                pygame.draw.rect(screen, GOLD, btn_auto, border_radius=3)
                screen.blit(font_tiny.render("MAX", True, BLACK), (btn_auto.x + 3, btn_auto.y + 3))
            else:
                pygame.draw.rect(screen, GREEN if cash >= c_stat['upg_cost_auto'] else GRAY, btn_auto, border_radius=3)
                screen.blit(font_tiny.render("Auto", True, BLACK), (btn_auto.x + 3, btn_auto.y + 3))
            card_rects[b_name + "_auto"] = btn_auto # Save rect to click mapping
    ...
    screen.blit(font_small.render("Pygame-CE Engine (School PC Bypass Active)", True, GREEN), (10, HEIGHT - 30))
    
    return btn_power, btn_cap, close_rect, card_rects, btn_view_tree, btn_prestige_act, btn_abilities, btn_special, btn_top_rev, btn_bot_rev, btn_logout
...
def draw_abilities_menu():
    ...
    for i, (a_name, a_data) in enumerate(ABILITIES.items()):
        ...
        screen.blit(font_small.render(f"Tokens: {tokens}", True, WHITE if tokens > 0 else RED), (card.x + 15, card.y + 45))
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
            if state not in ["LOGIN", "AUTO_LOGIN"]: 
                save_game(sync=True)
            running = False
            
        elif state == "LOGIN":
            # --- LOGIN UI EVENT HANDLING ---
            user_box = pygame.Rect(WIDTH//2 - 150, HEIGHT//2 - 80, 300, 40)
            pass_box = pygame.Rect(WIDTH//2 - 150, HEIGHT//2 - 10, 300, 40)
            btn_login = pygame.Rect(WIDTH//2 - 150, HEIGHT//2 + 60, 140, 40)
            btn_reg = pygame.Rect(WIDTH//2 + 10, HEIGHT//2 + 60, 140, 40)
            btn_offline = pygame.Rect(WIDTH//2 - 100, HEIGHT//2 + 120, 200, 40)

            if event.type == pygame.MOUSEBUTTONDOWN:
                if is_authenticating: continue 
                
                if user_box.collidepoint(mx, my): active_input = "username"
                elif pass_box.collidepoint(mx, my): active_input = "password"
                elif btn_login.collidepoint(mx, my):
                    if input_username and input_password:
                        is_authenticating = True
                        auth_message = "Logging in... Please wait."
                        threading.Thread(target=auth_thread, args=("login", input_username, input_password), daemon=True).start()
                elif btn_reg.collidepoint(mx, my):
                    if input_username and input_password:
                        is_authenticating = True
                        auth_message = "Creating Account... Uploading Save..."
                        threading.Thread(target=auth_thread, args=("register", input_username, input_password), daemon=True).start()
                elif btn_offline.collidepoint(mx, my):
                    load_local_game()
                    check_offline_progress()
                    state = "PLAY"
                ...
                if active_input == "username":
                    ...
                    elif event.key != pygame.K_RETURN and event.key != pygame.K_TAB: input_username += event.unicode
                elif active_input == "password":
                    if event.key == pygame.K_BACKSPACE: input_password = input_password[:-1]
                    elif event.key != pygame.K_RETURN and event.key != pygame.K_TAB: input_password += event.unicode
            ...
            if state == "OFFLINE_SCREEN":
                ...
                # Check the new Stacked Auto Toggles
                if not clicked_ui:
                    auto_y = 70
                    for b_name, b_stat in ball_stats.items():
                        if b_stat['auto_drop_lvl'] > 0:
                            toggle_rect = pygame.Rect(WIDTH - 150, auto_y, 130, 35)
                            if toggle_rect.collidepoint(mx, my):
                                b_stat['auto_enabled'] = not b_stat.get('auto_enabled', True)
                                save_game()
                                clicked_ui = True
                                break
                            auto_y += 45
                        ...
                        if ab_rect.collidepoint(mx, my):
                            ...
                            if a_name in ['Fire Cursor', 'Vacuum Cursor']:
                                ...
                                if ability_inventory.get(a_name, 0) > 0:
                                    ability_inventory[a_name] -= 1
                                    for _ in range(10): spawn_ball('Bouncy', random.randint(50, WIDTH-50), 20, random.uniform(-5, 5), 0)
                                    active_ability_mode = None
                            elif a_name == 'Drone':
                                if ability_inventory.get(a_name, 0) > 0:
                                    ability_inventory[a_name] -= 1
                                ...
                                if active_ability_mode == a_name: active_ability_mode = None 
                                elif ability_inventory.get(a_name, 0) > 0: active_ability_mode = a_name 
                            break
                ...
                if not clicked_ui:
                    ...
                    if active_drones and my < HEIGHT - 80:
                        for d in active_drones:
                            d['state'] = 'delivering'; d['drop_x'] = mx; d['drop_y'] = my
                        if bounce_sound: bounce_sound.play()
                        ...
                        if ability_inventory.get('Midas Touch', 0) > 0 and my < HEIGHT - 80:
                            ...
                            for peg in pegs:
                                if peg['active'] and math.hypot(peg['x'] - mx, peg['y'] - my) < 80 and peg['type'] not in ['boss', 'stat']: 
                                    peg['type'] = 'gold'; peg['on_fire'] = False 
                            active_ability_mode = None
                    ...
                    elif active_ability_mode in ['Fire Cursor', 'Vacuum Cursor']: pass 
                    else:
                ...
                btn_power, btn_cap, close_btn, card_rects, btn_view_tree, btn_prestige_act, btn_abilities, btn_special, btn_top_rev, btn_bot_rev, btn_logout = draw_main_menu()
                stats = ball_stats[equipped_ball]
                ...
                elif close_btn.collidepoint(mx, my): state = "PLAY"; save_game()
                elif btn_view_tree.collidepoint(mx, my): state = "PRESTIGE_TREE"; view_only_tree = True 
                elif btn_prestige_act.collidepoint(mx, my): state = "CONFIRM_PRESTIGE"
                elif btn_abilities.collidepoint(mx, my): state = "ABILITIES_MENU"
                elif btn_special and btn_special.collidepoint(mx, my) and cash >= 20000:
                    cash -= 20000; stats['special_unlocked'] = True; save_game()
                elif btn_top_rev and btn_top_rev.collidepoint(mx, my):
                    top_lv = stats.get('base_top_revives', 0)
                    if stat_points >= top_lv + 1 and top_lv < 5:
                        stat_points -= top_lv + 1; stats['base_top_revives'] = top_lv + 1; save_game()
                elif btn_bot_rev and btn_bot_rev.collidepoint(mx, my):
                    bot_lv = stats.get('base_bounce_revives', 0)
                    if stat_points >= bot_lv + 1 and bot_lv < 5:
                        stat_points -= bot_lv + 1; stats['base_bounce_revives'] = bot_lv + 1; save_game()
                elif btn_power.collidepoint(mx, my) and cash >= stats['upg_cost_power']:
                    cash -= stats['upg_cost_power']; stats['level'] += 1; stats['gold_mult'] += 0.5; stats['bounce_bonus'] += 0.02
                    stats['upg_cost_power'] = int(stats['upg_cost_power'] * 1.5)
                elif btn_cap.collidepoint(mx, my) and cash >= stats['upg_cost_balls']:
                    cash -= stats['upg_cost_balls']; stats['max_balls'] += 1
                    stats['upg_cost_balls'] = int(stats['upg_cost_balls'] * 2.5)
                    
                # Process card clicks (equip and auto upgrades)
                for key, rect in card_rects.items():
                    if rect.collidepoint(mx, my):
                        if "_auto" in key:
                            b_name = key.replace("_auto", "")
                            c_stat = ball_stats[b_name]
                            if cash >= c_stat['upg_cost_auto'] and c_stat['auto_drop_lvl'] < 4:
                                cash -= c_stat['upg_cost_auto']
                                c_stat['auto_drop_lvl'] += 1
                                c_stat['upg_cost_auto'] = int(c_stat['upg_cost_auto'] * 3.0)
                        else:
                            b_name = key
                            if ball_stats[b_name]['unlocked']: equipped_ball = b_name
                            elif cash >= BALL_TYPES[b_name]['cost']:
                                cash -= BALL_TYPES[b_name]['cost']
                                ball_stats[b_name]['unlocked'] = True; equipped_ball = b_name
                            
            elif state == "ABILITIES_MENU":
                a_rects, btn_unequip, btn_crates, btn_close = draw_abilities_menu()
                if btn_close.collidepoint(mx, my): state = "MENU"; save_game()
                elif btn_crates.collidepoint(mx, my): state = "CRATES_MENU"
                elif btn_unequip.collidepoint(mx, my):
                    equipped_abilities.clear(); active_ability_mode = None
                else:
                ...
                if btn_close.collidepoint(mx, my): state = "ABILITIES_MENU"
                else:
                ...
                if yes_rect.collidepoint(mx, my):
                    perform_prestige_reset(); state = "PRESTIGE_TREE"; view_only_tree = False 
                elif no_rect.collidepoint(mx, my): state = "MENU"

            elif state == "PRESTIGE_TREE":
                btn_close, hovered_node = draw_prestige_tree(mx, my)
                if btn_close.collidepoint(mx, my):
                    state = "MENU" if view_only_tree else "PLAY"; save_game()
                    ...
                    if p_upgrades.get(hovered_node['key'], 0) == hovered_node['level'] - 1:
                        if prestige_points >= hovered_node['cost']:
                            prestige_points -= hovered_node['cost']; p_upgrades[hovered_node['key']] += 1
                            active_coords = [(p['x'], p['y']) for p in pegs if p['active']]
                            populate_pegs(active_coords)

    if state == "AUTO_LOGIN":
        screen.fill(DARK_BG)
        pygame.draw.rect(screen, BLACK, (WIDTH//2 - 200, HEIGHT//2 - 100, 400, 200), border_radius=15)
        pygame.draw.rect(screen, GOLD, (WIDTH//2 - 200, HEIGHT//2 - 100, 400, 200), 3, border_radius=15)
        screen.blit(font_large.render("Authenticating...", True, CYAN), (WIDTH//2 - 120, HEIGHT//2 - 30))
        screen.blit(font_small.render(f"Logging in as {input_username}", True, GRAY), (WIDTH//2 - 100, HEIGHT//2 + 20))

    elif state == "LOGIN":
        screen.fill(DARK_BG)
        pygame.draw.rect(screen, BLACK, (WIDTH//2 - 200, HEIGHT//2 - 200, 400, 400), border_radius=15)
        pygame.draw.rect(screen, CYAN, (WIDTH//2 - 200, HEIGHT//2 - 200, 400, 400), 3, border_radius=15)
        ...
        pygame.draw.rect(screen, WHITE, user_box, border_radius=5)
        pygame.draw.rect(screen, CYAN if active_input == "username" else GRAY, user_box, 3, border_radius=5)
        screen.blit(font_small.render("Username:", True, GRAY), (WIDTH//2 - 150, HEIGHT//2 - 105))
        screen.blit(font_med.render(input_username, True, BLACK), (user_box.x + 10, user_box.y + 10))

        pygame.draw.rect(screen, WHITE, pass_box, border_radius=5)
        pygame.draw.rect(screen, CYAN if active_input == "password" else GRAY, pass_box, 3, border_radius=5)
        screen.blit(font_small.render("Password:", True, GRAY), (WIDTH//2 - 150, HEIGHT//2 - 35))
        hidden_pass = "*" * len(input_password)
        screen.blit(font_med.render(hidden_pass, True, BLACK), (pass_box.x + 10, pass_box.y + 10))
        ...
        pygame.draw.rect(screen, GREEN if not is_authenticating else GRAY, btn_login, border_radius=5)
        screen.blit(font_med.render("Login", True, BLACK), (btn_login.x + 35, btn_login.y + 10))
        
        pygame.draw.rect(screen, PURPLE if not is_authenticating else GRAY, btn_reg, border_radius=5)
        screen.blit(font_med.render("Register", True, WHITE), (btn_reg.x + 20, btn_reg.y + 10))
        
        pygame.draw.rect(screen, DARK_GRAY if not is_authenticating else GRAY, btn_offline, border_radius=5)
        screen.blit(font_small.render("Play Offline (Local Save)", True, WHITE), (btn_offline.x + 10, btn_offline.y + 12))

        # Messages
        if auth_message:
            msg_color = GREEN if "success" in auth_message.lower() or "created" in auth_message.lower() else (CYAN if is_authenticating else RED)
            msg_surface = font_small.render(auth_message, True, msg_color)
            screen.blit(msg_surface, (WIDTH//2 - msg_surface.get_width()//2, HEIGHT//2 + 170))
        ...
        for d in drones[:]:
            if d['state'] == 'collecting':
                if len(d['balls']) >= 10: d['state'] = 'holding'; d['target'] = None
                elif not d.get('target') or d['target'] not in balls:
                    if balls: d['target'] = random.choice(balls)
                    else: d['target'] = None

                if d['target'] and d['target'] in balls:
                    d['target']['vx'] *= 0.5; d['target']['vy'] *= 0.5
                    ...
                    d['y'] = max(60, d['y'] - 2); d['target'] = None 
                    
            elif d['state'] == 'holding': d['y'] = max(60, d['y'] - 2)
                ...
                if dist < speed:
                    for b in d['balls']:
                        b['x'], b['y'] = d['x'], d['y']; b['vx'] = random.uniform(-4, 4); b['vy'] = random.uniform(-2, 4); balls.append(b)
                    spawn_particles(d['x'], d['y'], CYAN, 25, 6); drones.remove(d)
                else: d['x'] += (dx/dist) * speed; d['y'] += (dy/dist) * speed
        ...
        if active_ability_mode == 'Vacuum Cursor':
            ...
            if vacuum_cursor_timer <= 0: active_ability_mode = None
                ...
                for b in balls:
                    ...
                    if dist < 200:
                        ...
                        b['vx'] += (dx/dist) * pull * 0.2; b['vy'] += (dy/dist) * pull * 0.2
                        b['vx'] *= 0.95; b['vy'] *= 0.95
                        
        if active_ability_mode == 'Fire Cursor':
            fire_cursor_timer -= 1
            if fire_cursor_timer <= 0: active_ability_mode = None
            elif pygame.mouse.get_pressed()[0] and my < HEIGHT - 80:
                ...
                if fire_ability_timer >= 2: 
                    ...
                    for peg in active_pegs:
                        if not peg.get('on_fire') and peg['type'] not in ['boss', 'stat'] and math.hypot(peg['x'] - mx, peg['y'] - my) < 30:
                            peg['on_fire'] = True; peg['fire_timer'] = FPS * 4; peg['fire_mult'] = 1.0; peg['fire_budget_ref'] = [2] 
        ...
        for bh in black_holes[:]:
            ...
            for peg in pegs:
                if peg['active']:
                    dx = bh['x'] - peg['x']; dy = bh['y'] - peg['y']
                    dist = math.hypot(dx, dy)
                    
                    if dist < bh['radius']:
                        pull_force = min(12.0, bh['radius'] / max(dist, 1)) 
                        if peg['type'] != 'boss':
                            peg['x'] += (dx / dist) * pull_force; peg['y'] += (dy / dist) * pull_force
                        
                        if dist < 20: 
                            if peg['type'] == 'boss':
                                peg['hp'] -= 1
                                if peg['hp'] <= 0: peg['active'] = False; grant_peg_reward(peg, 2.0, is_direct_hit=True) 
                            else:
                                peg['active'] = False; grant_peg_reward(peg, 2.0, is_direct_hit=False) 
                                spawn_particles(peg['x'], peg['y'], PURPLE, count=5, speed=6.0)
                            if random.random() < 0.2 and bounce_sound: bounce_sound.play()
            if bh['timer'] <= 0: black_holes.remove(bh)
        
        for c in clouds[:]:
            c['timer'] -= 1
            if c['timer'] <= FPS * 0.75 and not c['struck']:
                c['struck'] = True
                target = None; min_dist = float('inf')
                for p in active_pegs:
                    d = math.hypot(p['x'] - c['x'], p['y'] - c['y'])
                    if d < min_dist: min_dist = d; target = p
                ...
                if target:
                    ...
                    if target['type'] == 'boss':
                        ...
                        if target['hp'] <= 0: target['active'] = False; grant_peg_reward(target, 1.0, is_direct_hit=True)
                    else:
                        target['on_fire'] = True; target['fire_timer'] = FPS * 4; target['fire_budget_ref'] = [8]; target['fire_mult'] = 1.0 
                        for other in pegs:
                            if other['active'] and other['type'] not in ['boss', 'stat'] and not other.get('on_fire'):
                                if math.hypot(other['x'] - target['x'], other['y'] - target['y']) < 60:
                                    other['on_fire'] = True; other['fire_timer'] = FPS * 4; other['fire_mult'] = 1.0
                                    node_budget = target['fire_budget_ref']; other['fire_budget_ref'] = node_budget; node_budget[0] -= 1
            if c['timer'] <= 0: clouds.remove(c)
        
        if board_clear_timer == 0:
            for peg in pegs:
                if peg['active'] and peg.get('on_fire'):
                    peg['fire_timer'] -= 1
                    if random.random() < 0.1: spawn_particles(peg['x'], peg['y'], ORANGE, count=1, speed=2.0)
                    if random.random() < 0.05 and peg.get('fire_budget_ref', [0])[0] > 0:
                        for other in pegs:
                            if other['active'] and other['type'] not in ['boss', 'stat'] and not other.get('on_fire'):
                                if math.hypot(peg['x'] - other['x'], peg['y'] - other['y']) < 65:
                                    other['on_fire'] = True; other['fire_timer'] = FPS * 4; other['fire_mult'] = peg.get('fire_mult', 1.0)
                                    node_budget = peg['fire_budget_ref']; other['fire_budget_ref'] = node_budget; node_budget[0] -= 1; break 
                    if peg['fire_timer'] <= 0:
                        peg['active'] = False
                        if bounce_sound: bounce_sound.play()
                ...
                if b_stat['unlocked'] and b_stat['auto_drop_lvl'] > 0 and b_stat.get('auto_enabled', True):
                    ...
                    if b_stat['auto_timer'] >= rate * FPS:
                        ...
                        if active_pegs and random.random() < 0.8:
                            target_peg = random.choice(active_pegs); spawn_x = target_peg['x'] + random.uniform(-15, 15)
                        else: spawn_x = random.randint(20, WIDTH-20)
        ...
        for b in balls[:]:
            if b['type'] == 'Wood':
                ...
                if b['wood_timer'] >= FPS * 1.5: 
                    b['wood_timer'] = 0; cash += int(10 * b['gold_mult']); spawn_particles(b['x'], b['y'], GREEN, count=3, speed=2.0)
                    
            b['vy'] += b['grav']; b['x'] += b['vx']; b['y'] += b['vy']
            speed = math.hypot(b['vx'], b['vy']); max_speed = 18.0
                ...
                if b['lightning_timer'] <= 0:
                    ...
                    if target:
                        ...
                        if target['type'] == 'boss':
                            target['hp'] -= 1
                            spawn_particles(target['x'], target['y'], MAGENTA, count=10, speed=5.0)
                            if target['hp'] <= 0: target['active'] = False; grant_peg_reward(target, b['gold_mult'], is_direct_hit=True)
                        else:
                            target['on_fire'] = True; target['fire_timer'] = FPS * 4; target['fire_budget_ref'] = [5]; target['fire_mult'] = b['gold_mult']
                            for other in pegs:
                                if other['active'] and other['type'] not in ['boss', 'stat'] and not other.get('on_fire'):
                                    if math.hypot(other['x'] - target['x'], other['y'] - target['y']) < 60:
                                        other['on_fire'] = True; other['fire_timer'] = FPS * 4; other['fire_mult'] = b['gold_mult']
                                        node_budget = target['fire_budget_ref']; other['fire_budget_ref'] = node_budget; node_budget[0] -= 1
                        b['lightning_strikes'] -= 1; b['lightning_timer'] = FPS * 1.5 
                        if b['lightning_strikes'] <= 0:
                            b['type'] = 'Regular'; b['color'] = BALL_TYPES['Regular']['color']
                            b['grav'] = BALL_TYPES['Regular']['base_grav']; b['bounce'] = BALL_TYPES['Regular']['base_bounce'] + ball_stats['Regular']['bounce_bonus']
            ...
            if b['x'] - rad < 0 or b['x'] + rad > WIDTH:
                ...
                b['vx'] *= -b['bounce']
                if bounce_sound: bounce_sound.play()
                if b['type'] == 'Regular': cash += 5; spawn_particles(b['x'], b['y'], GOLD, count=3)
                elif b['type'] == 'Bouncy': b['gold_mult'] += 0.5; spawn_particles(b['x'], b['y'], CYAN, count=5)
                
            if b['y'] - rad > HEIGHT:
                if b.get('revive_stack'):
                    rev_type = b['revive_stack'].pop()
                    if rev_type == 'top':
                        b['y'] = 40; b['vy'] = 0.0; spawn_particles(b['x'], b['y'], CYAN, count=15, speed=6.0)
                        if bounce_sound: bounce_sound.play()
                        continue
                    elif rev_type == 'bounce':
                        b['y'] = HEIGHT - rad - 5; b['vy'] = -15.0; spawn_particles(b['x'], b['y'], GREEN, count=15, speed=6.0)
                        if bounce_sound: bounce_sound.play()
                        continue
                    
                if b['type'] == 'Maroon' and not b.get('bottom_bounced'):
                    b['y'] = HEIGHT - rad - 5; b['vy'] = -15.0; b['bottom_bounced'] = True
                    if bounce_sound: bounce_sound.play()
                    continue
                if b['type'] == 'Hoops' and not b.get('teleported'):
                    b['y'] = 40; b['vy'] = 0.0; b['teleported'] = True; spawn_particles(b['x'], HEIGHT, ORANGE, count=10); continue
                if b['type'] == 'Boulder':
                    if active_pegs:
                        ...
                        for t in targets:
                            if t['type'] == 'boss':
                                t['hp'] -= 1
                                if t['hp'] <= 0: t['active'] = False; grant_peg_reward(t, b['gold_mult'], True, 'Boulder')
                            else: t['active'] = False; grant_peg_reward(t, b['gold_mult'], True, 'Boulder')
                        if bounce_sound: bounce_sound.play()
                ...
                if peg['active']:
                    ...
                    if dist < rad + p_rad:
                        ...
                        if b['type'] == 'Taco':
                            ...
                            nearby = []
                            for p in pegs:
                                if p['active'] and p != peg:
                                    n_dist = math.hypot(b['x']-p['x'], b['y']-p['y'])
                                    if n_dist < 80:
                                        nearby.append((n_dist, p))
                                        
                            nearby.sort(key=lambda item: item[0])
                            eaten_pegs = [peg] + [n[1] for n in nearby[:2]]
                            
                            for ep in eaten_pegs:
                                if ep['type'] == 'boss':
                                    ep['hp'] -= 1
                                    spawn_particles(ep['x'], ep['y'], MAGENTA, count=10, speed=5.0)
                                    if ep['hp'] <= 0: 
                                        ep['active'] = False; grant_peg_reward(ep, b['gold_mult'], True, b['type'])
                                else:
                                    ep['active'] = False; grant_peg_reward(ep, b['gold_mult'], True, b['type'])
                                spawn_particles(ep['x'], ep['y'], YELLOW, 10, 4.0)

                            if b['hits_taken'] >= b.get('max_hits', 3):
                                balls.remove(b)
                                break 

                            if dist == 0: dist = 0.1 
                            nx, ny = dx / dist, dy / dist
                            b['x'] += nx * ((rad + p_rad) - dist); b['y'] += ny * ((rad + p_rad) - dist)
                            dot = b['vx'] * nx + b['vy'] * ny
                            b['vx'] = (b['vx'] - 2 * dot * nx) * b['bounce']; b['vy'] = (b['vy'] - 2 * dot * ny) * b['bounce']
                            break
                        ...
                        elif peg['type'] == 'stat': peg['active'] = False; grant_peg_reward(peg, b['gold_mult'], True, b['type'])
                        else:
                            if b['type'] == 'Fire':
                                if not peg.get('on_fire'):
                                    peg['on_fire'] = True; peg['fire_timer'] = FPS * 4; peg['fire_mult'] = b['gold_mult']; peg['fire_budget_ref'] = [random.randint(4, 6)] 
                            elif b['type'] == 'Bomb':
                                peg['active'] = False; grant_peg_reward(peg, b['gold_mult'], True, b['type']); spawn_particles(peg['x'], peg['y'], DARK_BLUE, count=15, speed=6.0)
                                for _ in range(8): spawn_ball('Shrapnel', peg['x'], peg['y'], random.uniform(-10, 10), random.uniform(-10, -2), inherited_mult=b['gold_mult'])
        ...
        if all(not p['active'] for p in pegs):
            if len(pegs) > 0 and board_clear_timer == 0:
                board_clear_timer = FPS * 2; cash += 100; boards_cleared += 1
        ...
        # --- Drawing ---
        for bmp in bumpers:
            points = [(bmp['x'], bmp['y'] - bmp['radius']), (bmp['x'] + bmp['radius'], bmp['y']), (bmp['x'], bmp['y'] + bmp['radius']), (bmp['x'] - bmp['radius'], bmp['y'])]
            pygame.draw.polygon(screen, GRAY, points); pygame.draw.polygon(screen, WHITE, points, 2)
            
        for bh in black_holes:
            bh_x, bh_y = int(bh['x']), int(bh['y']); pulse = math.sin(pygame.time.get_ticks() / 150.0) * 15
            ...
            pygame.draw.circle(screen, BLACK, (bh_x, bh_y), 20); pygame.draw.circle(screen, PURPLE, (bh_x, bh_y), 20, 2) 

        fx_surface.fill((0,0,0,0)) 
        for c in clouds:
            alpha = min(255, int((c['timer'] / FPS) * 255)); cx, cy = int(c['x']), int(c['y'])
            pygame.draw.circle(fx_surface, (*DARK_GRAY, alpha), (cx, cy), 35)
            pygame.draw.circle(fx_surface, (*DARK_GRAY, alpha), (cx - 25, cy + 15), 25)
            pygame.draw.circle(fx_surface, (*DARK_GRAY, alpha), (cx + 25, cy + 15), 25)
            
        for l in lasers:
            alpha = min(255, int((l['timer'] / FPS) * 255 * 2)); lx = int(l['x'])
            pygame.draw.rect(fx_surface, (*RED, alpha), (lx - 45, 0, 90, HEIGHT)); pygame.draw.rect(fx_surface, (*WHITE, alpha), (lx - 15, 0, 30, HEIGHT))
            
        screen.blit(fx_surface, (0, 0))
        
        for l in lightnings:
            pygame.draw.line(screen, PURPLE, l['start'], l['end'], 6); pygame.draw.line(screen, CYAN, l['start'], l['end'], 2)
        ...
        for peg in pegs:
            if peg['active']:
                ...
                else: pygame.draw.circle(screen, color, (int(peg['x']), int(peg['y'])), PEG_RADIUS)
        ...
        for d in drones:
            ...
            pygame.draw.circle(screen, CYAN, (dx - 15, dy - 5), 8, 2); pygame.draw.circle(screen, CYAN, (dx + 15, dy - 5), 8, 2)
            if d['balls']:
                pygame.draw.arc(screen, WHITE, (dx - 15, dy, 30, 20), math.pi, 0, 2)
                for i, b in enumerate(d['balls']):
                    bx = dx - 10 + (i % 5) * 5; by = dy + 5 + (i // 5) * 5; pygame.draw.circle(screen, b['color'], (bx, by), 3)

        if active_ability_mode == 'Thunder Cloud' and my < HEIGHT - 80:
            pygame.draw.circle(screen, WHITE, (mx, my), 35, 2); pygame.draw.circle(screen, WHITE, (mx - 25, my + 15), 25, 2); pygame.draw.circle(screen, WHITE, (mx + 25, my + 15), 25, 2)
        elif active_ability_mode == 'Fire Cursor' and my < HEIGHT - 80:
            pygame.draw.circle(screen, ORANGE, (mx, my), 30, 2); pygame.draw.circle(screen, RED, (mx, my), 4)
        elif active_ability_mode == 'Vacuum Cursor' and my < HEIGHT - 80:
            pygame.draw.circle(screen, CYAN, (mx, my), 200, 1); pygame.draw.circle(screen, WHITE, (mx, my), 10, 1)
            ...
            pygame.draw.circle(screen, PURPLE, (mx, my), 200, 2); pygame.draw.circle(screen, BLACK, (mx, my), 20); pygame.draw.circle(screen, PURPLE, (mx, my), 20, 2)
        elif active_ability_mode == 'Midas Touch' and my < HEIGHT - 80: pygame.draw.circle(screen, GOLD, (mx, my), 80, 2)
        elif active_ability_mode == 'Orbital Strike' and my < HEIGHT - 80:
            pygame.draw.rect(screen, RED, (mx - 45, 0, 90, HEIGHT), 2); pygame.draw.line(screen, RED, (mx, 0), (mx, HEIGHT), 1)
        ...
        for b in balls: 
            # --- CUSTOM TACO RENDER ---
            if b['type'] == 'Taco':
                b_rad = b.get('radius', 14)
                bx, by = int(b['x']), int(b['y'])
                health = 1.0 - (b.get('hits_taken', 0) / b.get('max_hits', 3))
                
                pygame.draw.circle(screen, GOLD, (bx, by), b_rad)
                pygame.draw.rect(screen, BROWN, (bx - b_rad + 2, by - 2, b_rad * 2 - 4, 6))
                pygame.draw.rect(screen, GREEN, (bx - b_rad + 4, by - 6, b_rad * 2 - 8, 4))
                
                # Draw black circles to represent missing bites
        ...
        # --- DRAW STACKED AUTO TOGGLES ---
        auto_y = 70
        for b_name, b_stat in ball_stats.items():
            if b_stat['auto_drop_lvl'] > 0:
                is_on = b_stat.get('auto_enabled', True)
                toggle_rect = pygame.Rect(WIDTH - 150, auto_y, 130, 35)
                pygame.draw.rect(screen, GREEN if is_on else RED, toggle_rect, border_radius=5)
                short_name = b_name[:6].upper()
                screen.blit(font_small.render(f"{short_name}: {'ON' if is_on else 'OFF'}", True, BLACK), (WIDTH - 140, auto_y + 10))
                auto_y += 45

        if equipped_abilities:
            for i, a_name in enumerate(equipped_abilities):
                tokens = ability_inventory.get(a_name, 0); is_active = (active_ability_mode == a_name)
        ...
        if board_clear_timer > 0:
            ...
            if board_clear_timer == 0:
                create_random_board(); balls.clear(); particles.clear(); lightnings.clear()
                clouds.clear(); black_holes.clear(); lasers.clear(); drones.clear()

    elif state == "MENU": draw_main_menu()
    elif state == "ABILITIES_MENU": draw_abilities_menu()
    elif state == "CRATES_MENU": draw_crates_menu()

    elif state == "CRATE_REWARD":
        screen.fill(BLACK)
        screen.blit(font_huge.render("CRATE OPENED!", True, GOLD), (WIDTH//2 - 220, 100))
        cols = 5; rows = math.ceil(len(crate_results_display) / cols)
        start_y = HEIGHT // 2 - (rows * 60)
        for i, res in enumerate(crate_results_display):
            r = i // cols; c = i % cols
            x = WIDTH//2 - (cols * 100) + (c * 200) + 100; y = start_y + (r * 150)
            ...
            if len(words) > 1: screen.blit(font_small.render(words[0], True, WHITE), (x - 60, y + 45)); screen.blit(font_small.render(words[1], True, WHITE), (x - 60, y + 70))
        ...
        yes_rect = pygame.Rect(WIDTH//2 - 150, HEIGHT//2 + 20, 100, 50); pygame.draw.rect(screen, GREEN, yes_rect, border_radius=5)
        screen.blit(font_med.render("YES", True, BLACK), (yes_rect.x + 25, yes_rect.y + 15))
        no_rect = pygame.Rect(WIDTH//2 + 50, HEIGHT//2 + 20, 100, 50); pygame.draw.rect(screen, RED, no_rect, border_radius=5)
        screen.blit(font_med.render("NO", True, WHITE), (no_rect.x + 30, no_rect.y + 15))

    elif state == "PRESTIGE_TREE": draw_prestige_tree(mx, my)
